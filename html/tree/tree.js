﻿/**
 * 宇燕树js，基于原生js，不需依赖任何现目前js框架，浏览器兼容性强，支持现目前市面上的主流浏览器（IE6+、firefox、chrome...），
 * 没有扩展及封装js原生对象（object、array），因此不会与其他js框架冲突，是您开发的首选树组件。
 * 树说明：
 *		1、右键菜单使用方法，方法名称：Tree.contextmenu,该方法可接受5个参数,为：node,event,actions,top,left
 *			node	:	被选中的节点
 *			event	：	window事件对象
 *			actions	:	右键菜单action数组,格式为[[{id:"action主键",name:"action显示名称",ico:"图标名称,图标路径请存放在Tree的images路径下",click:"单击action时触发",childs:[当此action存在子action时]},...],[...]],
 *						此actions为二维数组,二维数组的作用可实现action的区域化显示
 *			top		:	右键菜单距页面顶部的距离,此参数可选,如果不设置则去window事件对象中的eventY
 *			left	：	右键菜单距页面左部的距离,此参数可选,如果不设置则去window事件对象中的eventX
 *	@author 李云龙
 *	@version 1.000
 */
/**
 ...
 *	@class Tree
 *	@constructor
 *	@param {String} id 树id(非页面显示树div的id)，请保证每棵树的id唯一性
 */
